            ...
            switch (gcfg->srctype) {
                ...
                case (MCX_SRC_GAUSSIAN): { // uniform disk distribution or collimated Gaussian-beam
                    ...
                    if (gcfg->srctype == MCX_SRC_DISK || gcfg->srctype == MCX_SRC_RING) {
                        ...
                        sincosf(phi,&sphi,&cphi);
                        float r = launchsrc->param1.x;

                        float ytheta = 2.f*ONE_PI*launchsrc->param1.y/360.f;
                        float ptheta = 2.f*ONE_PI*launchsrc->param1.z/360.f;
                        float rtheta = 2.f*ONE_PI*launchsrc->param1.w/360.f;

                        float Cy = cosf(ytheta);
                        float Sy = sinf(ytheta);
                        float Cp = cosf(ptheta);
                        float Sp = sinf(ptheta);
                        float Cr = cosf(rtheta);
                        float Sr = sinf(rtheta);

                        float x1 = p->x + r*cphi;//cphi*0.5;
                        float y1 = p->y + r*sphi;//sphi*0.5;
                        float z1 = p->z + launchsrc->param2.x/2.f - rand_uniform01(t)*launchsrc->param2.x;

                        float x2 = x1 - p->x;
                        float y2 = y1 - p->y;
                        float z2 = z1 - p->z;

                        float x3=Cy*Cp*x2+(Cy*Sp*Sr-Sy*Cr)*y2+(Cy*Sp*Cr+Sy*Sr)*z2;
                        float y3=Sy*Cp*x2+(Sy*Sp*Sr+Cy*Cr)*y2+(Sy*Sp*Cr-Cy*Sr)*z2;
                        float z3=-Sp*x2+Cp*Sr*y2+Cp*Cr*z2;
                        ...
                        float ztheta = ONE_PI/2.f - ONE_PI*rand_uniform01(t);
                        float thetaXY = phi +ONE_PI/2.f - ONE_PI*rand_uniform01(t);

                        float Cxy = cosf(thetaXY);
                        float Sxy = sinf(thetaXY);
                        float Cz = cosf(ztheta);
                        float Sz = sinf(ztheta);

                        float vxx = Cz * Cxy;
                        float vyy = Cz * Sxy;
                        float vzz = Sz;

                        float vx=Cy*Cp*vxx+(Cy*Sp*Sr-Sy*Cr)*vyy+(Cy*Sp*Cr+Sy*Sr)*vzz;
                        float vy=Sy*Cp*vxx+(Sy*Sp*Sr+Cy*Cr)*vyy+(Sy*Sp*Cr-Cy*Sr)*vzz;
                        float vz=(-Sp)*vxx+Cp*Sr*vyy+Cp*Cr*vzz;
